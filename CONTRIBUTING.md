...
spec/
  ...
  CATEGORIES.md              dac / preamp / amplifier / streamer / ...
  ...
  "controls": [
    {
      ...
      "source": "https://acme.example/A100-manual.pdf#page=31",
      "verified": true
    }
  ],
...
**Every control needs a `source`.** A URL to the manufacturer's manual, with a
page or section where possible. A command with no provenance is unreviewable —
nobody can tell a transcription slip from a typo, and a wrong power command sent
to a stranger's amplifier is not a cosmetic bug. This is enforced by the
validator, not by reviewers' goodwill.
...
## 3. Devices you cannot send a command to
